import { FastifyRequest, FastifyReply } from 'fastify'
import { z } from 'zod'
import { MakeGetMonthExpenseBySectorUseCase } from '@/modules/sectors/use-cases/factories/make-get-month-expense-by-sector'

export async function getMonthExpenseBySector(request: FastifyRequest, reply: FastifyReply) {
    const getExpensesQuerySchema = z.object({
        month: z.string().optional().transform(m => m ? Number(m) : undefined),
        year: z.string().optional().transform(y => y ? Number(y) : undefined),
    })

    const { month, year } = getExpensesQuerySchema.parse(request.query)
    const date = (month !== undefined && year !== undefined) ? new Date(year, month - 1, 1) : undefined

    let monthExpenseBySector
    try {
        const getMonthExpenseBySectorUseCase = MakeGetMonthExpenseBySectorUseCase()
        monthExpenseBySector = await getMonthExpenseBySectorUseCase.execute(date)
    } catch (err) {
        if (err instanceof Error) {
            return reply.status(409).send({ message: err.message })
        }

        throw err
    }
    return reply.status(200).send(monthExpenseBySector)
}


