...
// src/repositories/in-memory/in-memory-transactions-repository.spec.ts
var sut;
describe("InMemoryTransactionsRepository", () => {
  ...
  describe("getFinancialSummary", () => {
    it("should calculate total balance correctly based on confirmed transactions", async () => {
      ...
      globalExpect(summary.totalBalance).toBe(700);
    });
    it("should separate monthly incomes and expenses correctly", async () => {
      ...
      const summary = await sut.getFinancialSummary();
      globalExpect(summary.monthlyIncome).toBe(500);
      globalExpect(summary.monthlyExpenses).toBe(100);
    });
    it("should calculate pending incomes and expenses for the current month", async () => {
      ...
      const summary = await sut.getFinancialSummary();
      globalExpect(summary.pendingIncome).toBe(200);
      globalExpect(summary.pendingExpenses).toBe(50);
    });
    it("should identify overdue transactions correctly", async () => {
      ...
      const summary = await sut.getFinancialSummary();
      globalExpect(summary.overdueIncome).toBe(150);
      globalExpect(summary.overdueExpenses).toBe(80);
    });
  });
  describe("findMany", () => {
    it("should filter transactions by month", async () => {
      ...
      globalExpect(resultJan?.transactions).toHaveLength(1);
      globalExpect(resultJan?.transactions[0].id).toBe("tx-jan");
    });
  });
});
