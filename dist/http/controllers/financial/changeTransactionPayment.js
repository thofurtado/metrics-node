...
var changeTransactionPayment_exports = {};
__export(changeTransactionPayment_exports, {
  changeTransactionStatus: () => changeTransactionStatus
});
module.exports = __toCommonJS(changeTransactionPayment_exports);
...
async function changeTransactionStatus(request, reply) {
  const switchTransactionParamsSchema = import_zod2.z.object({
    id: import_zod2.z.string().uuid()
  });
  const switchTransactionBodySchema = import_zod2.z.object({
    amount: import_zod2.z.number().positive(),
    date: import_zod2.z.string().or(import_zod2.z.date()).transform((val) => new Date(val)),
    // Data de liquidação
    // NOVO: remainingDate é opcional e deve ser uma data
    remainingDate: import_zod2.z.string().or(import_zod2.z.date()).transform((val) => new Date(val)).optional(),
    // NOVO: Permitir trocar a conta na hora do pagamento
    account_id: import_zod2.z.string().uuid().optional()
  });
