...
var DeleteItemUseCase = class {
  constructor(itemsRepository2, stocksRepository2) {
...
// src/use-cases/delete-item.spec.ts
var itemsRepository;
...
describe("Delete Item Use Case", () => {
  ...
  it("should be able to delete an item without stock history", async () => {
    ...
    globalExpect(deletedItem).toBeNull();
  });
  it("should not be able to delete an item with stock history", async () => {
    ...
    await globalExpect(sut.execute({ itemId: item.id })).rejects.toThrow("Cannot delete item with stock history. Archive it instead.");
  });
  it("should not be able to delete a non-existent item", async () => {
    await globalExpect(sut.execute({ itemId: "non-existent-id" })).rejects.toBeInstanceOf(ResourceNotFoundError);
  });
});
