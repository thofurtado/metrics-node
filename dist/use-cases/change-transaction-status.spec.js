...
var ChangeTransactionUseCase = class {
  constructor(transactionsRepository2) {
...
// src/use-cases/change-transaction-status.spec.ts
...
describe("Change Transaction Status Use Case", () => {
  ...
  it("should be able to settle a transaction fully", async () => {
    ...
    globalExpect(updatedTransaction?.confirmed).toBe(true);
    globalExpect(updatedTransaction?.amount).toBe(1e3);
    globalExpect(updatedTransaction?.date).toEqual(/* @__PURE__ */ new Date("2024-01-05"));
  });
  it("should be able to settle a transaction partially (Split)", async () => {
    ...
    globalExpect(updatedOriginal?.confirmed).toBe(true);
    globalExpect(updatedOriginal?.amount).toBe(400);
    const allTransactions = transactionsRepository.items;
    const remainingTransaction = allTransactions.find((t) => t.id !== transaction.id);
    globalExpect(remainingTransaction).toBeDefined();
    globalExpect(remainingTransaction?.amount).toBe(600);
    globalExpect(remainingTransaction?.confirmed).toBe(false);
    globalExpect(remainingTransaction?.description).toBe("PR (1): Servi\xE7o");
    globalExpect(remainingTransaction?.operation).toBe("income");
  });
  it("should correctly increment the partial payment level description", async () => {
    const transaction = await transactionsRepository.create({
      description: "PR (1): Servi\xE7o",
      amount: 600,
      operation: "income",
      account_id: "acc-1",
      confirmed: false,
    ...
    const allTransactions = transactionsRepository.items;
    const remainingTransaction = allTransactions.find((t) => t.id !== transaction.id);
    globalExpect(remainingTransaction?.description).toBe("PR (2): Servi\xE7o");
    globalExpect(remainingTransaction?.amount).toBe(400);
  });
  it("should fallback description if original has none", async () => {
    ...
    const allTransactions = transactionsRepository.items;
    const remainingTransaction = allTransactions.find((t) => t.id !== transaction.id);
    globalExpect(remainingTransaction?.description).toBe("PR (1): Sem Descri\xE7\xE3o Original");
  });
  ...
  it("should not be able to settle with negative amount", async () => {
    ...
    await globalExpect(sut.execute({
      ...
      amount: -100,
      date: /* @__PURE__ */ new Date()
    })).rejects.toThrow("O valor de liquida\xE7\xE3o (amount) deve ser positivo.");
  });
