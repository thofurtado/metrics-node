...
var GetInventorySummaryUseCase = class {
  constructor(inventoryRepository2) {
    this.inventoryRepository = inventoryRepository2;
  }
  async execute() {
...
var InMemoryInventoryRepository = class {
  ...
  async getInventorySummary() {
    ...
    const productsSoldValue = monthTreatmentItems.reduce((sum, treatmentItem) => {
      const item = this.items.find((i2) => i2.id === treatmentItem.item_id);
      if (item?.isItem) {
...
// src/use-cases/get-inventory-summary.spec.ts
var inventoryRepository;
var getInventorySummaryUseCase;
describe("Get Inventory Summary Use Case", () => {
  ...
  it("should be able to get inventory summary with correct value calculations", async () => {
    ...
    globalExpect(inventorySummary.patrimony).toEqual(1e3);
    globalExpect(inventorySummary.productsSold).toEqual(310);
    globalExpect(inventorySummary.servicesSold).toEqual(200);
  });
  it("should calculate patrimony only for active items with isItem true", async () => {
    ...
    inventoryRepository.createItem({
      ...
      cost: 200,
      stock: 1,
      // 200 * 1 = 200 (mas não conta)
      isItem: false,
      active: true
    });
    const { inventorySummary } = await getInventorySummaryUseCase.execute();
    globalExpect(inventorySummary.patrimony).toEqual(500);
  });
  it("should only count treatment items from current month", async () => {
    ...
    inventoryRepository.createTreatmentItem({
      ...
      quantity: 2,
      salesValue: 100
      // Valor: 2 * 100 = 200 (não conta)
    });
    const { inventorySummary } = await getInventorySummaryUseCase.execute();
    globalExpect(inventorySummary.productsSold).toEqual(300);
  });
  it("should handle items without salesValue (use price as fallback)", async () => {
    ...
    const { inventorySummary } = await getInventorySummaryUseCase.execute();
    globalExpect(inventorySummary.productsSold).toEqual(160);
  });
  it("should return zero values when no data exists", async () => {
    const { inventorySummary } = await getInventorySummaryUseCase.execute();
    globalExpect(inventorySummary.patrimony).toEqual(0);
    globalExpect(inventorySummary.productsSold).toEqual(0);
    globalExpect(inventorySummary.servicesSold).toEqual(0);
  });
  it("should correctly separate products and services sales", async () => {
    ...
    const { inventorySummary } = await getInventorySummaryUseCase.execute();
    globalExpect(inventorySummary.productsSold).toEqual(100);
    globalExpect(inventorySummary.servicesSold).toEqual(150);
  });
});
