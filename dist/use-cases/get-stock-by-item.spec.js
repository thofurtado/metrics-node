...
// src/use-cases/get-stock-by-item.spec.ts
var stocksRepository;
var getStocksUseCase;
var itemsRepository;
describe("Get Stocks By Item Use Case", () => {
  ...
  it("should be able to get stocks by item", async () => {
    ...
    const input = stocks?.filter((item2) => item2.operation === "input");
    const output = stocks?.filter((item2) => item2.operation === "output");
    const totalInput = input?.reduce((total, item2) => total + item2.quantity, 0);
    const totalOutput = output?.reduce((total, item2) => total + item2.quantity, 0);
    if (totalInput !== void 0 && totalOutput !== void 0)
      globalExpect(totalInput - totalOutput).toEqual(1);
    globalExpect(stocks?.length).toEqual(4);
  });
});
