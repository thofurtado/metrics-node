...
var FinishTreatmentUseCase = class {
  constructor(treatmentsRepository2, paymentEntrysRepository2, itemsRepository2, transactionsRepository2, accountsRepository2) {
    this.treatmentsRepository = treatmentsRepository2;
    this.paymentEntrysRepository = paymentEntrysRepository2;
    this.itemsRepository = itemsRepository2;
    this.transactionsRepository = transactionsRepository2;
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (paymentEntries) {
        for (const entry of paymentEntries) {
          ...
          for (let i2 = 0; i2 < entry.occurrences; i2++) {
            const dueDate = /* @__PURE__ */ new Date();
            dueDate.setMonth(dueDate.getMonth() + i2);
            ...
            const transaction = await this.transactionsRepository.create({
              ...
              description: `Atendimento #${treatment.id} - ${paymentMethod.name} (${i2 + 1}/${entry.occurrences})`,
...
var InMemoryTransactionsRepository = class {
  ...
  async create(data) {
    const transaction = {
      id: data.id ? data.id : (0, import_node_crypto4.randomUUID)(),
...
// src/use-cases/finish-treatment.spec.ts
var mockTx = {
  stock: {
    create: vi.fn()
  }
...
describe("Finish Treatment Use Case", () => {
  ...
  it("should be able to finish a treatment normally (No Contract)", async () => {
    ...
    const treatment = await treatmentsRepository.create({
      status: "pending",
      amount: 100,
      opening_date: /* @__PURE__ */ new Date(),
      request: "Format"
    });
    const t = treatmentsRepository.items.find((t2) => t2.id === treatment.id);
    if (t) {
    ...
    const pe = paymentEntrysRepository.items.find((p) => p.id === paymentEntry.id);
    if (pe) {
      pe.payments = {
        id: "pid",
        ...
        name: "Dinheiro",
        in_sight: true
        /* Mocking the payment method object itself */
      };
    }
    const response = await sut.execute({ treatment_id: treatment.id });
    globalExpect(response.treatment.status).toBe("resolved");
    const updatedItem = await itemsRepository.findById(item.id);
    globalExpect(updatedItem?.stock).toBe(9);
    const txs = transactionsRepository.items;
    globalExpect(txs).toHaveLength(1);
    globalExpect(txs[0].amount).toBe(100);
  });
  it("should be able to finish a CONTRACT treatment with 0 balance", async () => {
    ...
    globalExpect(response.treatment.status).toBe("resolved");
    const txs = transactionsRepository.items;
    globalExpect(txs).toHaveLength(0);
  });
  it("should be able to finish a CONTRACT treatment with Parts charged separately", async () => {
    ...
    globalExpect(updatedSSD?.stock).toBe(4);
    const txs = transactionsRepository.items;
    globalExpect(txs).toHaveLength(1);
    globalExpect(txs[0].amount).toBe(100);
  });
  it("should block finish if amount > 0 and no payment (Normally)", async () => {
    ...
    await globalExpect(sut.execute({ treatment_id: treatment.id })).rejects.toThrow("Pagamento insuficiente");
  });
