...
var TreatmentUseCase = class {
  constructor(treatmentsRepository2, clientsRepository2, equipmentsRepository2, usersRepository2) {
    this.treatmentsRepository = treatmentsRepository2;
    this.clientsRepository = clientsRepository2;
    this.equipmentsRepository = equipmentsRepository2;
...
var InMemoryEquipmentsRepository = class {
  ...
  async findMany(type2, brand, identification) {
    let filteredEquipments = this.items.slice();
    if (type2 !== void 0) {
      filteredEquipments = filteredEquipments.filter((item) => item.type === type2);
    }
    if (brand !== void 0) {
...
describe("Treatment Use Case", () => {
  ...
  it("should be able to create treatment", async () => {
    const { treatment } = await treatmentUseCase.execute({
      ...
      observations: "O computador de cima n\xE3o da sinal no monitor, thais j\xE1 tentou retirar os cabos, passei o passo a passo para desernegizar"
    });
    globalExpect(treatment.id).toEqual(globalExpect.any(String));
  });
  it("should not be able to create a treatment with an invalid client", async () => {
    await globalExpect(treatmentUseCase.execute({
      contact: "Thais",
      request: "Computador esta sem sinal",
      observations: "O computador de cima n\xE3o da sinal no monitor, thais j\xE1 tentou retirar os cabos, passei o passo a passo para desernegizar",
      client_id: "client.id"
