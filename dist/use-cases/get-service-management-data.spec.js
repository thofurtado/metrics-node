...
var require_type_detect = __commonJS({
  "node_modules/type-detect/type-detect.js"(exports2, module2) {
    ...
    (function(global3, factory) {
      typeof exports2 === "object" && typeof module2 !== "undefined" ? module2.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global3 = typeof globalThis !== "undefined" ? globalThis : global3 || self, global3.typeDetect = factory());
    })(exports2, (function() {
      "use strict";
      ...
      _replaceString(string3, replacement) {
        ...
        if (index !== -1) {
          if (typeof replacement === "function") {
            replacement = replacement(string3, index, original);
          }
          if (string3 !== replacement) {
...
var GetServiceManagementDataUseCase = class {
  constructor(serviceManagementRepository2) {
    this.serviceManagementRepository = serviceManagementRepository2;
  }
  async execute() {
...
var InMemoryServiceManagementRepository = class {
  ...
  async getServiceManagementData() {
    ...
    const now3 = currentDate.getTime();
    ...
    const averageTreatmentTime = this.calculateAverageTime(monthTreatments, now3);
    return {
      totalTreatments,
      completedTreatments,
      inWorkbench,
  ...
  calculateAverageTime(treatments, now3) {
    ...
    const totalTime = treatments.reduce((sum, treatment) => {
      ...
      const end = treatment.ending_date ? new Date(treatment.ending_date).getTime() : now3;
...
// src/use-cases/get-service-management-data.spec.ts
var serviceManagementRepository;
var getServiceManagementDataUseCase;
function createTreatment(repository, treatment) {
  const newTreatment = {
...
describe("Get Service Management Data Use Case", () => {
  ...
  it("should be able to get service management data with correct calculations", async () => {
    ...
    createTreatment(serviceManagementRepository, {
      ...
      ending_date: new Date(currentDate.getFullYear(), currentDate.getMonth(), 4, 10, 0),
      // 3 dias depois
      status: "resolved",
    ...
    createTreatment(serviceManagementRepository, {
      ...
      status: "in_workbench",
      request: "Troca de pe\xE7as"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: new Date(currentDate.getFullYear(), currentDate.getMonth(), 3, 14, 0),
      status: "in_progress",
      request: "Visita t\xE9cnica"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: new Date(currentDate.getFullYear(), currentDate.getMonth(), 4, 8, 0),
      status: "pending",
      request: "Aguardando aprova\xE7\xE3o"
    });
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.totalTreatments).toEqual(4);
    globalExpect(serviceData.completedTreatments).toEqual(1);
    globalExpect(serviceData.inWorkbench).toEqual(1);
    globalExpect(serviceData.externalOpen).toEqual(2);
    globalExpect(serviceData.averageTreatmentTime).toBeGreaterThan(0);
  });
  it("should correctly count workbench and external treatments from all months", async () => {
    ...
    createTreatment(serviceManagementRepository, {
      ...
      status: "resolved",
      request: "Finalizado este m\xEAs"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: new Date(currentDate.getFullYear(), currentDate.getMonth(), 2),
      status: "in_workbench",
      request: "Bancada este m\xEAs"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: lastMonth,
      status: "in_workbench",
      // Deve contar para bancada
      request: "Bancada m\xEAs passado"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: lastMonth,
      status: "in_progress",
      // Deve contar para externos
      request: "Externo m\xEAs passado"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: lastMonth,
      status: "pending",
      // Deve contar para externos
      request: "Pendente m\xEAs passado"
    });
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.totalTreatments).toEqual(2);
    globalExpect(serviceData.completedTreatments).toEqual(1);
    globalExpect(serviceData.inWorkbench).toEqual(2);
    globalExpect(serviceData.externalOpen).toEqual(2);
  });
  it("should be able to get service management data with no treatments", async () => {
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.totalTreatments).toEqual(0);
    globalExpect(serviceData.completedTreatments).toEqual(0);
    globalExpect(serviceData.inWorkbench).toEqual(0);
    globalExpect(serviceData.externalOpen).toEqual(0);
    globalExpect(serviceData.averageTreatmentTime).toEqual(0);
  });
  it("should correctly calculate average time for completed treatments", async () => {
    const currentDate = /* @__PURE__ */ new Date();
    createTreatment(serviceManagementRepository, {
      opening_date: new Date(currentDate.getTime() - 2 * 24 * 60 * 60 * 1e3),
      // 2 dias atrás
      ending_date: new Date(currentDate.getTime() - 1 * 24 * 60 * 60 * 1e3),
      // 1 dia atrás
      status: "resolved",
      request: "Reparo r\xE1pido"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: new Date(currentDate.getTime() - 3 * 24 * 60 * 60 * 1e3),
      // 3 dias atrás
      ending_date: new Date(currentDate.getTime() - 2 * 24 * 60 * 60 * 1e3),
      // 2 dias atrás
      status: "resolved",
      request: "Manuten\xE7\xE3o preventiva"
    });
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.averageTreatmentTime).toBeCloseTo(2160, -2);
  });
  it("should correctly identify external open treatments", async () => {
    ...
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.externalOpen).toEqual(3);
    globalExpect(serviceData.totalTreatments).toEqual(7);
  });
  it("should include open treatments in average time calculation", async () => {
    const currentDate = /* @__PURE__ */ new Date();
    const twoDaysAgo = new Date(currentDate.getTime() - 2 * 24 * 60 * 60 * 1e3);
    createTreatment(serviceManagementRepository, {
    ...
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.averageTreatmentTime).toBeCloseTo(2880, -2);
  });
  it("should calculate average time only for current month treatments", async () => {
    ...
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.totalTreatments).toEqual(1);
    globalExpect(serviceData.averageTreatmentTime).toBeGreaterThan(0);
  });
  it("should count all treatments resolved this month regardless of when they started", async () => {
    ...
    createTreatment(serviceManagementRepository, {
      ...
      // Finalizado este mês
      status: "resolved",
      request: "Finalizado este m\xEAs - come\xE7ou m\xEAs passado"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: new Date(currentDate.getFullYear(), currentDate.getMonth(), 1),
      ending_date: new Date(currentDate.getFullYear(), currentDate.getMonth(), 3),
      status: "resolved",
      request: "Iniciado e finalizado este m\xEAs"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: twoMonthsAgo,
      ending_date: new Date(currentDate.getFullYear(), currentDate.getMonth(), 10),
      // Finalizado este mês
      status: "resolved",
      request: "Finalizado este m\xEAs - come\xE7ou h\xE1 2 meses"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: twoMonthsAgo,
      ending_date: new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 15),
      // Finalizado mês passado
      status: "resolved",
      request: "Finalizado m\xEAs passado"
    });
    createTreatment(serviceManagementRepository, {
      opening_date: lastMonth,
      status: "in_progress",
    ...
    const { serviceData } = await getServiceManagementDataUseCase.execute();
    globalExpect(serviceData.totalTreatments).toEqual(1);
    globalExpect(serviceData.completedTreatments).toEqual(3);
    globalExpect(serviceData.inWorkbench).toEqual(0);
    globalExpect(serviceData.externalOpen).toEqual(1);
  });
