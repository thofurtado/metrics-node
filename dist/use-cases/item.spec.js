...
var ItemUseCase = class {
  constructor(itemsRepository2, stockRepository) {
...
describe("Item Use Case", () => {
  ...
  it("should be able to create an item", async () => {
    ...
    globalExpect(item.id).toEqual(globalExpect.any(String));
  });
  it("should be able to create an item with stock value", async () => {
    ...
    globalExpect(item.id).toEqual(globalExpect.any(String));
    globalExpect(stock.length).toEqual(1);
  });
  it("should not be able to create an item with the same name", async () => {
    ...
    await globalExpect(itemUseCase.execute({
      name: "Mouse Usb 2.0 Multilaser",
      cost: 9.5,
      price: 17.3,
      stock: 2
