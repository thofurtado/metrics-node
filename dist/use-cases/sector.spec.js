...
var SectorUseCase = class {
  constructor(sectorsRepository2) {
    this.sectorsRepository = sectorsRepository2;
  }
  async execute({
    name,
    budget,
...
describe("Sector Use Case", () => {
  ...
  it("should be able to create a sector", async () => {
    ...
    globalExpect(sector.id).toEqual(globalExpect.any(String));
  });
  it("should not be able to create a sector with the same name as other one", async () => {
    ...
    await globalExpect(sectorUseCase.execute({
      name: "fiscal",
      budget: 100
